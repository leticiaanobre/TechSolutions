...
interface ClientState {
  ...
  createTask: (taskData: { name: string; description: string; priority: string; estimatedHours: number; dueDate: Date }) => Promise<void>;
...
export const useClientStore = create<ClientState>((set) => ({
  ...
  fetchTasks: async () => {
    ...
    try {
      const response = await axiosInstance.get('/api/v1/clients/tasks');  // Usando axiosInstance
